        ...
        <div class="flex justify-center md:m-5">
            <div class="flex flex-col justify-center">
                ...
                <img src="/img/garden.jpg"
                    ...
                    v-if="!selected.gardenReadOnly & !selected.apartment" @click="setGarden()" />
            ...
            <div class="flex flex-col justify-between place-items-center">
                ...
                <div class="flex justify-center place-items-center">
                    ...
                    <button :disabled="selected.getNbNeeded() == 0" class="btn btn-sm md:btn-md btn-ghost"
                        @click="clearFoods()">
                ...
                <div class="flex">
                    <div class="indicator">
                        <span class="indicator-item badge badge-secondary" v-if="selected.nbBurger">{{
                            selected.nbBurger
                        }}</span>
                        <svg width="55px" viewBox="85 130 40 40"
                            :class="['cursor-pointer', selected.nbBurger ? '' : 'grayscale opacity-50']"
                            @click="setFoods('burger')">
                    ...
                    <div class="indicator">
                        <span class="indicator-item badge badge-secondary" v-if="selected.nbPizza">{{
                            selected.nbPizza
                        }}</span>
                        <svg width="55px" viewBox="85 130 40 40"
                            :class="['cursor-pointer', selected.nbPizza ? '' : 'grayscale opacity-50']"
                            @click="setFoods('pizza')">
                    ...
                    <div class="indicator">
                        <span class="indicator-item badge badge-secondary" v-if="selected.nbBeer">{{
                            selected.nbBeer
                        }}</span>
                        <svg width="55px" viewBox="85 130 40 40"
                            :class="['cursor-pointer', selected.nbBeer ? '' : 'grayscale opacity-50']"
                            @click="setFoods('beer')">
                    ...
                    <div class="indicator">
                        <span class="indicator-item badge badge-secondary" v-if="selected.nbCoke">{{
                            selected.nbCoke
                        }}</span>
                        <svg width="55px" viewBox="85 130 40 40"
                            :class="['cursor-pointer', selected.nbCoke ? '' : 'grayscale opacity-50']"
                            @click="setFoods('coke')">
                    ...
                    <div class="indicator">
                        <span class="indicator-item badge badge-secondary" v-if="selected.nbJuice">{{
                            selected.nbJuice
                        }}</span>
                        <svg width="55px" viewBox="85 130 40 40"
                            :class="['cursor-pointer', selected.nbJuice ? '' : 'grayscale opacity-50']"
                            @click="setFoods('juice')">
...
const setGarden = () => {
    selected.value.garden = !selected.value.garden;
}
const setPark = () => {
    selected.value.park = !selected.value.park;
}
const setFoods = (food) => {
    switch (food) {
        case 'burger':
            selected.value.nbBurger++;
            break;
        case 'pizza':
            selected.value.nbPizza++;
            break;
        case 'beer':
            selected.value.nbBeer++;
            break;
        case 'coke':
            selected.value.nbCoke++;
            break;
        case 'juice':
            selected.value.nbJuice++;
            break;
    }
}
const clearFoods = () => {
    selected.value.nbBurger = 0;
    selected.value.nbPizza = 0;
